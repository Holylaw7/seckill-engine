...
| 项 | 状态 | 说明 |
| --- | --- | --- |
| A1 远程 CI | ⏳ 等待外部资源方 | 需提供远程仓库 URL 与 Actions 凭据 |
| A2 独立 Load Generator | ⏳ 等待外部资源方 | 需提供独立主机/容器 |
| A3 生产规格 RocketMQ | ⏳ 等待外部资源方 | 需提供 Namesrv/Broker 集群 |
| A4 生产数据中心 | ⏳ 等待外部资源方 | 需提供部署窗口 |
| A5 运营 Owner | ⏳ 等待外部资源方 | 需指派并签署 |
| C1 收敛轮询连接池 | ✅ 已完成 | IntegrationTestBase.pooledQueryInt（HikariCP 懒加载，默认行为不变）；IntegrationEnvironmentSmokeIT 3/3 验证 PASS（commit 8b3da79） |
| C2 / C3 | ✅ 已完成 | 前置阶段已就绪 |
